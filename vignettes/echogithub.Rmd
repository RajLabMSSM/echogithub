...
You can also search for hex stickers. This can be useful if you want to embed
the URL in your README files, for example.
By default, it will wrap the hex sticker URL in an HTML element 
for easy embedding.

```{r, results="asis"}
hex <- echogithub::github_hex(owner ="RajLabMSSM", 
                              repo = "echolocatoR",
                              add_html = TRUE)
cat(hex)
```
...
`description_find` searches a variety of sources 
(e.g. local files, local installations, GitHub) to find the correct 
*DESCRIPTION* file and import it into the easy-to-use `packageDescription` format.

## Find 

```{r}
desc_file <- echogithub::description_find(owner = "RajLabMSSM", 
                                          repo = "echolocatoR")
methods::show(desc_file)
```
...
```{r}
res <- echogithub::description_extract(desc_file = desc_file)
methods::show(res)
```
